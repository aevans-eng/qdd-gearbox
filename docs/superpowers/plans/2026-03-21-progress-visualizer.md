# Progress Visualizer Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a CustomTkinter dashboard that reads `progress.json` and displays learning progress, unlocks, checkpoints, and next actions.

**Architecture:** Single-file Python script (`ui/progress_visualizer.py`) reads `testing/learn/progress.json`, computes derived state (next action, countdowns, progress bars), and renders a two-panel CustomTkinter window. Data refreshes every 60s or on window focus.

**Tech Stack:** Python 3.10+, customtkinter 5.2.2, json/datetime (stdlib)

**Spec:** `docs/superpowers/specs/2026-03-21-progress-visualizer-design.md`
**Mockup:** `.superpowers/brainstorm/118471-1774144951/final-mockup.html`
**Data:** `testing/learn/progress.json`
...
| File | Responsibility |
|------|---------------|
| `ui/progress_visualizer.py` | Entire application — data loading, computation, UI rendering, refresh loop |
| `testing/learn/progress.json` | Data source (read-only, not modified by this tool) |

Single file. No tests — this is a read-only display tool with hardcoded layout. Visual verification against the mockup is the test.

---

### Task 1: Window Shell + Data Loading

**Files:**
- Create: `ui/progress_visualizer.py`
...
def get_next_networking_action(data):
    ...
    # No unlocked ones — show first locked
    for key, udata in data["unlocks"].items():
...
def get_next_unlock_progress(data):
    """Return (unlock_label, completed_count, total_required, flag_info) or None."""
    for key, udata in data["unlocks"].items():
        if not udata.get("unlocked", False):
...
Run: `python ui/progress_visualizer.py`
Expected: Script exits cleanly (no window yet, no mainloop)

- [ ] **Step 3: Commit**

```
git add ui/progress_visualizer.py
git commit -m "feat: progress visualizer — data loading and computation helpers"
```

---

### Task 2: Main Window + Two-Panel Layout

**Files:**
- Modify: `ui/progress_visualizer.py`

- [ ] **Step 1: Add the App class with two-panel layout**

Append to `progress_visualizer.py`:
...
class App(ctk.CTk):
    ...
    def build_layout(self):
        ...
        self.grid_columnconfigure(0, weight=2)  # main ~65%
        self.grid_columnconfigure(1, weight=1)  # sidebar ~35%
    ...
    # Placeholder methods — filled in by subsequent tasks
    def build_hero(self): pass
    def build_journey(self): pass
    def build_sidebar_streak(self): pass
    def build_sidebar_checkpoints(self): pass
    def build_sidebar_progress(self): pass
    def build_sidebar_unlocks(self): pass
...
- [ ] **Step 2: Run the script to verify the window opens**

Run: `python ui/progress_visualizer.py`
Expected: Dark window opens at 960x720 with empty main panel and sidebar. Close it manually.

- [ ] **Step 3: Commit**

```
git add ui/progress_visualizer.py
git commit -m "feat: progress visualizer — window shell with two-panel layout"
```

---

### Task 3: Hero Action Cards

**Files:**
- Modify: `ui/progress_visualizer.py`

- [ ] **Step 1: Implement build_hero()**

Replace the `build_hero` placeholder:
...
```python
    def build_hero(self):
        ...
        if net:
            ...
            self._action_card(
                ...
                border="#2dd4bf44" if unlocked else "#55555544",
            )
    ...
    def _action_card(self, parent, icon, title, subtitle, btn_text,
        ...
        btn = ctk.CTkLabel(inner, text=btn_text, font=("Segoe UI", 10, "bold"),
                           fg_color=accent, text_color="#ffffff" if accent != C_TEAL else "#000000",
                           corner_radius=14, width=70, height=26)
        btn.grid(row=0, column=2, rowspan=2, padx=(12, 0))
```

- [ ] **Step 2: Run and verify hero cards appear**

Run: `python ui/progress_visualizer.py`
Expected: Two action cards at top — blue learning card showing "Distill — 01 Motor Fundamentals" and teal networking card showing "First outreach to Francis & Onur"

- [ ] **Step 3: Commit**

```
git add ui/progress_visualizer.py
git commit -m "feat: progress visualizer — hero action cards"
```

---

### Task 4: Sidebar — Streak + Checkpoints + Progress Bars

**Files:**
- Modify: `ui/progress_visualizer.py`

- [ ] **Step 1: Implement sidebar streak, checkpoints, and progress bars**

Replace the three sidebar placeholders:
...
```python
    def build_sidebar_streak(self):
        ...
        row.pack(anchor="w")
        ctk.CTkLabel(row, text="🔥", font=("Segoe UI", 16)).pack(side="left")
        ctk.CTkLabel(row, text=str(streak["current"]),
                     font=("Segoe UI", 26, "bold"), text_color=C_GOLD).pack(side="left", padx=(4, 0))

        ctk.CTkLabel(frame, text=f"day streak · best: {streak['longest']}",
                     font=("Segoe UI", 10), text_color=C_TEXT_SEC).pack(anchor="w")
    ...
    def build_sidebar_checkpoints(self):
        ...
        for label, target, color in CHECKPOINTS:
            ...
            # Left border simulation via inner padding
            ...
            lbl_font = ctk.CTkFont(family="Segoe UI", size=10, weight="bold",
                                    overstrike=done)
            lbl = ctk.CTkLabel(inner, text=label, font=lbl_font,
                               text_color=color if not done else C_TEXT_MUTED, anchor="w")
            lbl.grid(row=0, column=0, sticky="w", padx=(8, 0), pady=(4, 0))

            date_str = target.strftime("%b %d") if label != "SENIOR CONTACTS" else f"Before {target.strftime('%b %d')}"
            ctk.CTkLabel(inner, text=date_str, font=("Segoe UI", 8),
                         text_color=C_TEXT_SEC, anchor="w").grid(row=1, column=0, sticky="w", padx=(8, 0), pady=(0, 4))

            day_text = f"{days}d" if days > 0 else "DONE"
            ctk.CTkLabel(inner, text=day_text, font=("Segoe UI", 14, "bold"),
                         text_color=color if not done else C_TEXT_MUTED).grid(row=0, column=1, rowspan=2, padx=(0, 10))

    def _cp_bg(self, color):
        """Return a dark background tint based on checkpoint color."""
        bg_map = {
            C_RED: "#2a1a1e", C_PURPLE: "#1f1a2e", C_YELLOW: "#1f1f0e",
            C_GREEN: "#0f1f15", C_ORANGE: "#2a1a0e",
        }
    ...
    def build_sidebar_progress(self):
        ...
        if nup:
            ...
            self._progress_bar(frame, "Next unlock", pct, f"{done_count}/{total}", C_BLUE)

        # Total unlocks
        unlocked, total = get_unlock_stats(self.data)
        pct = unlocked / total if total > 0 else 0
        self._progress_bar(frame, "Total unlocks", pct, f"{unlocked}/{total}", C_GREEN)
    ...
    def _progress_bar(self, parent, label, fraction, text, color):
        ...
        ctk.CTkLabel(row, text=label, font=("Segoe UI", 9),
                     text_color=C_TEXT_SEC, width=70, anchor="w").grid(row=0, column=0)

        bar = ctk.CTkProgressBar(row, progress_color=color, fg_color="#333333",
                                 height=5, corner_radius=3)
        bar.set(fraction)
        bar.grid(row=0, column=1, sticky="ew", padx=4)

        ctk.CTkLabel(row, text=text, font=("Segoe UI", 9),
                     text_color=C_TEXT_SEC, width=28, anchor="e").grid(row=0, column=2)
```

- [ ] **Step 2: Run and verify sidebar elements**

Run: `python ui/progress_visualizer.py`
Expected: Sidebar shows streak (🔥 2), 5 checkpoints with countdown days, and 2 progress bars

- [ ] **Step 3: Commit**

```
git add ui/progress_visualizer.py
git commit -m "feat: progress visualizer — sidebar streak, checkpoints, progress bars"
```

---

### Task 5: Sidebar — Unlock List

**Files:**
- Modify: `ui/progress_visualizer.py`

- [ ] **Step 1: Implement build_sidebar_unlocks()**
...
```python
    def build_sidebar_unlocks(self):
        ...
        for key, udata in self.data["unlocks"].items():
            ...
            dot = ctk.CTkFrame(row, fg_color=dot_color, width=6, height=6, corner_radius=3)
            dot.grid(row=0, column=0, padx=(0, 6), pady=3)
            dot.grid_propagate(False)

            # Short label from full label
            short = udata["label"]
            if len(short) > 25:
                short = short[:22] + "..."
            ...
            if is_next:
                badge = ctk.CTkLabel(name_frame, text="NEXT", font=("Segoe UI", 7, "bold"),
                                     fg_color=C_BLUE, text_color="#ffffff",
                                     corner_radius=3, width=30, height=14)
            ...
            ctk.CTkLabel(row, text=date_str, font=("Segoe UI", 8),
                         text_color=C_TEXT_MUTED, anchor="e").grid(row=0, column=2)
```

- [ ] **Step 2: Run and verify unlock list**

Run: `python ui/progress_visualizer.py`
Expected: Sidebar shows all 12 unlocks. "First outreach" in green with "DONE". "Resume update" in blue with "NEXT" badge. Rest in grey.

- [ ] **Step 3: Commit**

```
git add ui/progress_visualizer.py
git commit -m "feat: progress visualizer — sidebar unlock list"
```

---

### Task 6: Journey Timeline

**Files:**
- Modify: `ui/progress_visualizer.py`

- [ ] **Step 1: Implement build_journey()**
...
```python
    def build_journey(self):
        ...
        # Divider
        ctk.CTkFrame(self.main_frame, fg_color="#ffffff08", height=1).pack(fill="x", padx=20, pady=(0, 4))
        ...
        for phase in PHASES:
            ...
            ctk.CTkLabel(row, text=phase["dates"], font=("Segoe UI", 10, date_weight),
                         text_color=date_color, width=78, anchor="e").grid(row=0, column=0, padx=(0, 8))
            ...
            dot = ctk.CTkFrame(dot_frame, fg_color=dot_color, width=10, height=10, corner_radius=5)
            ...
            if is_exam:
                ...
                ctk.CTkLabel(card, text="EXAMS — streak paused", font=("Segoe UI", 10),
            ...
            card = ctk.CTkFrame(row, fg_color=card_bg, corner_radius=8,
                                ...
                                border_color="#3b82f622" if is_current else "transparent")
            card.grid(row=0, column=2, sticky="ew", pady=2)
            ...
            for col, header in enumerate(["Learn", "Apply", "Ship"]):
                ctk.CTkLabel(inner, text=header, font=("Segoe UI", 9),
                             text_color="#666666", anchor="w").grid(row=0, column=col, sticky="w")

            # Learn column — topic IDs with progress dots
            ...
            for tid in phase["topics"]:
                ...
                if tdata:
                    ...
                    # Build colored dot string — green filled, grey empty
                    tid_label = ctk.CTkFrame(learn_frame, fg_color="transparent")
                    tid_label.pack(anchor="w")
                    ctk.CTkLabel(tid_label, text=f"{tid} ", font=("Segoe UI", 10),
                                 text_color=C_BLUE_LIGHT if is_current else C_TEXT_SEC,
                                 anchor="w").pack(side="left")
                    for d in dots:
                        ctk.CTkLabel(tid_label, text="●" if d else "○",
                                     font=("Segoe UI", 8),
                                     text_color=C_GREEN if d else C_TEXT_MUTED,
                                     anchor="w").pack(side="left")

            # Apply column
            apply_text = phase["apply"]
            apply_color = C_TEXT if is_current else C_TEXT_MUTED
            if apply_text:
                ctk.CTkLabel(inner, text=apply_text, font=("Segoe UI", 10),
                             text_color=apply_color, anchor="w").grid(row=1, column=1, sticky="w")
            ...
            for stype, stext in phase["ship"]:
                if stype == "done":
                    ctk.CTkLabel(ship_frame, text=f"✓ {stext}", font=("Segoe UI", 10),
                                 text_color=C_GREEN, anchor="w").pack(anchor="w")
                elif stype == "highlight":
                    ctk.CTkLabel(ship_frame, text=stext, font=("Segoe UI", 10),
                                 text_color=C_PURPLE, anchor="w").pack(anchor="w")
                elif stype == "milestone":
                    ctk.CTkLabel(ship_frame, text=f"★ {stext}", font=("Segoe UI", 10, "bold"),
                                 text_color=C_GREEN, anchor="w").pack(anchor="w")
                elif stype == "milestone_orange":
                    ctk.CTkLabel(ship_frame, text=f"★ {stext}", font=("Segoe UI", 10, "bold"),
                                 text_color=C_ORANGE, anchor="w").pack(anchor="w")
            if not phase["ship"]:
                ctk.CTkLabel(ship_frame, text="—", font=("Segoe UI", 10),
                             text_color=C_TEXT_MUTED).pack(anchor="w")
```

- [ ] **Step 2: Run and verify journey timeline**

Run: `python ui/progress_visualizer.py`
Expected: Journey timeline shows 7 phase rows with Learn/Apply/Ship columns. Current phase (Mar 22-25) highlighted blue. Exam row in yellow. Progress dots visible on topics 01-04.

- [ ] **Step 3: Commit**

```
git add ui/progress_visualizer.py
git commit -m "feat: progress visualizer — journey timeline"
```

---

### Task 7: Final Polish + Focus Refresh

**Files:**
- Modify: `ui/progress_visualizer.py`

- [ ] **Step 1: Add focus-based refresh**

Add to `__init__` after `self.schedule_refresh()`:

```python
        self.bind("<FocusIn>", self._on_focus)

    def _on_focus(self, event):
        if event.widget == self:  # Only root window, not child widgets
            self.refresh_data()
```
...
Compare against mockup at `.superpowers/brainstorm/118471-1774144951/final-mockup.html`. Verify:
1. Hero cards show correct next actions
2. Journey timeline has 7 rows with correct phase highlighting
3. Sidebar has streak, 5 checkpoints, 2 progress bars, 12 unlocks
4. Colors roughly match the mockup's dark theme
5. Window resizes without breaking layout

- [ ] **Step 3: Fix any visual issues found in step 2**

Adjust padding, fonts, colors as needed to match the mockup more closely.

- [ ] **Step 4: Final commit**

```
git add ui/progress_visualizer.py
git commit -m "feat: progress visualizer — polish and focus refresh"
```
