...
**Folder boundary:** this file is the hardware/test-concept reference: trainer limits, mechanical interface, adapter notes, and how trainer readings should be interpreted. The executable BLE capture tool lives in `testing/dyno/ble-capture/`.
...
| Protocols | ANT+ FE-C, BLE Cycling Power Service, proprietary BLE resistance control | Matches local H2 tooling |
...
**Data update rate:** ANT+ power profile is typically ~4 Hz. The local BLE CPS path on this H2 is about `~1.1 Hz`. Both are fine for steady-state measurements but too slow for transient dynamics (step response, cogging). For transients, continue using motor-controller telemetry.
...
### Option C: Bluetooth CPS + Proprietary Control (Alternative)

- `bleak` Python library for BLE
- Cycling Power Service exposes power, torque, and wheel revolution data
- Resistance control appears to use a Saris proprietary BLE characteristic
