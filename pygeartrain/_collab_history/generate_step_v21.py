"""
generate_step_v21.py - Tight Fit Herringbone Planetary Gearbox

Based on v20B with same teeth (R48/P18/S12) but:
- Steeper helix: 30° (was 20°) for better axial resistance
- Interference fit: +0.05mm (was -0.10mm clearance)

Goal: Zero backlash gearbox that relies on plastic squish during assembly.
"""
...
def validate_planetary_config(R, P, S, N):
    ...
    # Distance between adjacent planet centers: 2 * carrier_radius * sin(pi/N)
    # Must be greater than 2 * planet_outer_radius
    # In tooth units: (S + P) * sin(pi/N) > (P + 2)  [assuming addendum = 1 module]
    carrier_factor = S + P  # proportional to carrier radius
    planet_clearance_needed = P + 2  # proportional to planet outer diameter
    planet_spacing = carrier_factor * math.sin(math.pi / N)

    if planet_spacing <= planet_clearance_needed:
        # Calculate max planets that would work
        ...
        # Calculate min S that would work with current P and N
        # (S + P) * sin(pi/N) > P + 2
        # S > (P + 2) / sin(pi/N) - P
    ...
    # Rule 4: Minimum teeth (practical limit for involute gears)
    if S < 6:
...
# INTERFERENCE FIT - CHANGED: positive offset for tighter fit
PROFILE_OFFSET_MM = +0.05  # was -0.10 (clearance), now interference
RING_OFFSET_DEG = 0.2

# Ring wall thickness
RING_WALL_MM = 8.0

# M2 Self-threading screw parameters (same as v20B)
...
# Output directory
OUTPUT_DIR = "step_output_v21"

print("=" * 60)
print("STEP FILE GENERATOR v21 (Tight Fit, 30° Helix, Large Teeth)")
print("=" * 60)
...
print(f"\n--- v21 Changes from v20B ---")
print(f"Teeth: R{R_teeth}/P{P_teeth}/S{S_teeth} (was R48/P18/S12)")
print(f"Helix: {HELIX_ANGLE_DEGREES}° (was 20°)")
print(f"Fit: {PROFILE_OFFSET_MM:+.2f}mm interference (was -0.10mm clearance)")
print()
print(f"--- Geometry ---")
...
# For interference fit: ring internal space should be SMALLER (negative offset)
# Since PROFILE_OFFSET_MM is now positive, we negate it for the ring
...
print("\n" + "=" * 60)
print("v21 GENERATION COMPLETE")
print(f"Output directory: {OUTPUT_DIR}/")
print("\n--- Key Changes from v20B ---")
print(f"  Teeth: R{R_teeth}/P{P_teeth}/S{S_teeth} (larger teeth, {(R_teeth + S_teeth) / S_teeth:.1f}:1 ratio)")
print(f"  Helix: {HELIX_ANGLE_DEGREES}° (more axial resistance)")
print(f"  Fit: {PROFILE_OFFSET_MM:+.2f}mm interference (zero backlash goal)")
print("\n--- Files ---")
print("  - sun.step/.stl")
